...
class Individual:
    def __init__(self, data_fields):
        ...
        self.is_data_dict_copied = False
    ...
    def receive_feedback(self, success, time, prev_solution):
        if self.callback is not None:
            self.callback(success, time, prev_solution)

    def set_callback(self, func):
        self.callback = func

    def set_data_field(self, key, value):
        if not self.is_data_dict_copied:
            self.is_data_dict_copied = True
            self.data_fields = self.data_fields.copy()
