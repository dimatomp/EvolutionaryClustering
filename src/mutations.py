...
def get_all_moves(separation=None, cohesion=None):
    ...
    return ['unguided_merge_gene_move', 'unguided_remove_and_reclassify_move', 'unguided_split_gene_move',
            ...
            'prototype_hill_climbing_move', 'tree_hill_climbing_move', 'FastPointMove()'] + separation_moves + cohesion_moves
...
class FastPointMove:
    def __call__(self, indiv: Individual):
        if 'mutation_rate' not in indiv:
            indiv.set_data_field('mutation_rate', len(indiv['labels']) / 2)
        ...
        clusters, centroids = get_clusters_and_centroids(labels, data)
        centroid_distances = cdist(data, centroids)
        centroids_numbers = centroid_distances.argmin(axis=1)
        centroid_distances = centroid_distances.min(axis=1)

        # calculating the probabilities for the points to be moved
        probabilities = 1 / centroid_distances
        probabilities /= probabilities.sum()

        # choosing each point with probability that is inversely proportional to its distance to the nearest cluster
        to_mutate = np.random.choice(len(centroids_numbers), min(np.count_nonzero(probabilities), int(np.ceil(mutation_rate))), replace=False, p=probabilities)

        labels[to_mutate] = centroids_numbers[to_mutate]

        def callback(success, time, prev_solution):
            if success:
                indiv.set_data_field('mutation_rate', min(indiv['mutation_rate'] * 2 ** 0.25, len(labels) / 2))
                print('Increasing mutation rate', indiv['mutation_rate'], file=sys.stderr)
            else:
                prev_solution.set_data_field('mutation_rate', max(indiv['mutation_rate'] / 2, 1))
                print('Decreasing mutation rate', prev_solution['mutation_rate'], file=sys.stderr)

        indiv.set_callback(callback)
        return "Using mutation rate " + str(mutation_rate)
