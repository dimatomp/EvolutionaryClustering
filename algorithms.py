...
def run_one_plus_one(initialization, mutation, index, data, true_labels, boundary=1e-9, num_tries=10000, logging=None):
    c_solution = initialization(data, len(np.unique(true_labels)))
    ...
    while c_tries < num_tries:
        ...
        if is_better(n_index, c_index):
            ...
            ground_truth = adjusted_mutual_info_score(true_labels, c_solution["labels"])
